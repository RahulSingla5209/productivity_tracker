...
# Load
resp = supabase.table("activities").select("*").eq("user_id", user["user_id"]).order("date", desc=True).execute()
...
if df.empty:
    ...
    labels = [label_row(r) for _, r in df.iterrows()]
